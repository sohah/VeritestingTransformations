...
public class TestIntStatic1 {
	...
	public static void testInt1(int x, int y) {
		...
		if (x > 1) {
			assert pcMatches(I1_PC1, pc) : makePCAssertString("TestIntStatic1.testInt1 if x > 1", I1_PC1, TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert pcMatches(I1_PC2, pc) : makePCAssertString("TestIntStatic1.testInt1 x <= 1",
		...
		if (z > 30) {
			assert (pcMatches(I1_PC3, pc) || pcMatches(I1_PC4, pc)) : makePCAssertString(
					"TestIntStatic1.testInt1 z <= 30", "one of\n" + (I1_PC3 + " && " + pc) + "\nor\n"
