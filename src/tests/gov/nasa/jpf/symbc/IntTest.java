...
// x > 1
  ...
  protected static String I1_PC1;// = "x > 1";
  //
  // (x <= 1)
  protected static String I1_PC2;// = "x <= 1";
  //
  // [(x > 1) && ((z := y) > 30)] || [(x < 1) && ((z := x+y) > 30)] || [(x == 1) && ((z := x+y) > 30)]
  protected static String I1_PC3;// = "(y + x) > 30";
  protected static String I1_PC4;// = "y > 30";
  //
  // [((z := x+y) < 30) && (x == 1)] || [(x < 1) && ((z := x+y) < 30)] ||
  // [(x < 1) && ((z := x+y) == 30)] || [(x == 1) && ((z := x+y) == 30)] ||
  // [(x > 1) && ((z := y) < 30)] || [(x > 1) && ((z := y) == 30)]
  protected static String I1_PC5;// = "(y + x) <= 30";
  protected static String I1_PC6;// = "y <= 30";

  protected static void testInt(int x, int y) {

    I1_PC1 = TestPC.intPC1("x",">",1);
    I1_PC2 = TestPC.intPC1("x","<=",1);
    I1_PC3 = TestPC.intPC2("y","+","x",">",30);
    I1_PC4 = TestPC.intPC1("y",">",30);
    I1_PC5 = TestPC.intPC2("y","+","x","<=",30);
    I1_PC6 = TestPC.intPC1("y","<=",30);

    String pc = "";
    int z = x + y;

    if (x > 1) {
      assert pcMatches(I1_PC1) : makePCAssertString("TestIntSpecial1.testInt1 if x > 1", I1_PC1, 
