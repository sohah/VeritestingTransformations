...
public class TestDoubleVirtual1 {
	// x > 1.1
	...
	void testDouble1(double x, double y) {
		String pc = "";
		double z = x + y;

		if (x > 1.1) {
			assert pcMatches(PC1, pc) : makePCAssertString("TestDoubleVirtual1.testDouble1 if x > 1.1", PC1, TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert (pcMatches(PC2, pc) || pcMatches(PC3, pc)) : makePCAssertString(
					"TestDoubleVirtual1.testDouble1 x <= 1.1", "either\n" + PC2 + "\nor\n" + PC3, TestUtils
							.getPathCondition());
		}
		pc = trimPC(TestUtils.getPathCondition());
		if (z > 30.0) {
			assert (pcMatches(PC4, pc) || pcMatches(PC5, pc)) : makePCAssertString(
					"TestDoubleVirtual1.testDouble1 z <= 30.0", "one of\n" + (PC4 + " && " + pc) + "\nor\n"
