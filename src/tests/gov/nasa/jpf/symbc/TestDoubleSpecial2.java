...
public class TestDoubleSpecial2 {
	// x > 1.1
	...
	//
	// [(x > 1.1) && ((z := y) > 30)] || [(x < 1.1) && ((z := x+y) > 30)] || [(x == 1.1) && ((z := x+y) > 30)]
	private static String PC4 = "(x_1_SYMREAL + y_2_SYMREAL) > CONST_30.0";

	private static String PC5 = "y_2_SYMREAL > CONST_30.0";

	//
	// [((z := x+y) < 30) && (x == 1.1)] || [(x < 1.1) && ((z := x+y) < 30)] ||
	// [(x < 1.1) && ((z := x+y) == 30)] || [(x == 1.1) && ((z := x+y) == 30)] ||
	// [(x > 1.1) && ((z := y) < 30)] || [(x > 1.1) && ((z := y) == 30)]
	...
	private void test1(double x, double y) {
		String pc = trimPC(TestUtils.getPathCondition());
		double z = x + y;

		if (x > 1.1) {
			assert pcMatches(PC1, pc) : makePCAssertString("TestDoubleSpecial.test1 if x > 1.1f", (PC1 + " && " + pc), TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert (pcMatches(PC2, pc) || pcMatches(PC3, pc)) : makePCAssertString("TestDoubleSpecial.test1 x <= 1.1f",
					"either\n" + (PC2 + " && " + pc) + "\nor\n" + (PC3 + " && " + pc), TestUtils.getPathCondition());
		}
	...
	public static void main(String[] args) {
		TestDoubleSpecial2 test = new TestDoubleSpecial2();
		test.test1(11.0, 21.0);
	}
}
