...
public class TestIntSpecial1 {
	...
	private void testInt1(int x, int y) {
		String pc = "";
		int z = x + y;

		if (x > 1) {
			assert pcMatches(I1_PC1, pc) : makePCAssertString("TestIntSpecial1.testInt1 if x > 1", I1_PC1, TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert pcMatches(I1_PC2, pc) : makePCAssertString("TestIntSpecial1.testInt1 x <= 1",
