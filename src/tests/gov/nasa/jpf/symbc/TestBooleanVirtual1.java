...
public class TestBooleanVirtual1 {
	// (x == true)
	...
	void testBoolean1(boolean x, boolean y) {
		String pc = "";
		// Note: "!y" compiles to IFEQ, so it creates a choice generator
		boolean z = !y;

		if (x) {
			assert pcMatches(B1_PC1, pc) || pcMatches(B1_PC2, pc) : makePCAssertString(
					"TestBooleanVirtual1.testBoolean1 if (x == true)", "one of\n" + B1_PC1 + "\nor\n" + B1_PC2,
					TestUtils.getPathCondition());
			z = y;
		} else {
			assert pcMatches(B1_PC3, pc) || pcMatches(B1_PC4, pc) : makePCAssertString(
					"TestBooleanVirtual1.testBoolean1 (x == false)", "one of\n" + B1_PC3 + "\nor\n" + B1_PC4, TestUtils
							.getPathCondition());
		}
		pc = trimPC(TestUtils.getPathCondition());
		if (!z) {
			assert (pcMatches(B1_PC5, pc) || pcMatches(pc, "")) : makePCAssertString(
					"TestBooleanVirtual1.testBoolean1 (z == false)", "one of\n" + (B1_PC5 + " && " + pc) + "\nor\n"
							+ pc, TestUtils.getPathCondition());
			z = true;
		} else {
			assert (pcMatches(pc, "")) : makePCAssertString("TestBooleanVirtual1.testBoolean1 (z == true)", pc,
					TestUtils.getPathCondition());
		}
		pc = trimPC(TestUtils.getPathCondition());
	}
