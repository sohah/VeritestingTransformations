...
public class TestFloatSpecial1 {
	// x > 1.1f
	...
	private void testFloat1(float x, float y) {
		String pc = "";
		float z = x + y;

		if (x > 1.1f) {
			assert pcMatches(PC1, pc) : makePCAssertString("TestFloatSpecial1.testFloat1 if x > 1.1f", PC1, TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert (pcMatches(PC2, pc) || pcMatches(PC3, pc)) : makePCAssertString("TestFloatSpecial1.testFloat1 x <= 1.1f",
