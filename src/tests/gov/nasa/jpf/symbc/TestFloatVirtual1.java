...
public class TestFloatVirtual1 {
	// x > 1.1f
	...
	void testFloat1(float x, float y) {
		String pc = "";
		float z = x + y;

		if (x > 1.1f) {
			assert pcMatches(PC1, pc) : makePCAssertString("TestFloatVirtual1.testFloat1 if x > 1.1f", PC1, TestUtils
					.getPathCondition());
			z = y;
		} else {
			assert (pcMatches(PC2, pc) || pcMatches(PC3, pc)) : makePCAssertString(
					"TestFloatVirtual1.testFloat1 x <= 1.1f", "either\n" + PC2 + "\nor\n" + PC3, TestUtils
							.getPathCondition());
		}
		pc = trimPC(TestUtils.getPathCondition());
		if (z > 30.0f) {
			assert (pcMatches(PC4, pc) || pcMatches(PC5, pc)) : makePCAssertString(
					"TestFloatVirtual1.testFloat1 z <= 30.0f", "one of\n" + (PC4 + " && " + pc) + "\nor\n"
