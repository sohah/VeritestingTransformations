...
public abstract class Parser implements CommandLineParser
{
     ...
     *                        while parsing the command line tokens.
     */
    public CommandLine parse(Options options, char[][] arguments) throws ParseException
    {
     ...
     * @since 1.1
     */
    public CommandLine parse(Options options, char[][] arguments, Properties properties) throws ParseException
    {
     ...
     */
    public CommandLine parse(Options options, char[][] arguments, boolean stopAtNonOption) throws ParseException
    {
     ...
     * @since 1.1
     */
    public CommandLine parse(Options options, char[][] arguments, Properties properties, boolean stopAtNonOption)
            throws ParseException
    {
...
//        List<String> tokenList = Arrays.asList(flatten(getOptions(), arguments, stopAtNonOption));
        char[][] _flatten = flatten(getOptions(), arguments, stopAtNonOption);
    ...
    protected void processProperties(Properties properties) throws ParseException
    {
        ...
        for (Enumeration<?> e = properties.propertyNames(); e.hasMoreElements();)
        {
            ...
            Option opt = options.getOption(option.toCharArray());
            if (opt == null)
            {
                throw new UnrecognizedOptionException("Default option wasn't defined", option.charAt(0));
            }
            ...
            if (!cmd.hasOption(option) && !selected)
            {
                ...
                if (opt.hasArg())
                {
                    if (opt.hasNoValues())
                    {
                        try
                        {
                            opt.addValueForProcessing(value.toCharArray());
                        }
    ...
    public void processArgs(Option opt, ListIterator iter) throws ParseException
    {
        ...
        while (iter.hasNext())
        {
            ...
            if (getOptions().hasOption(str) && str[0] == '-')
            {
                iter.previous();
                break;
            }
        ...
        if (opt.hasNoValues() && !opt.hasOptionalArg())
        {
    ...
    protected void processOption(char[] arg, ListIterator iter) throws ParseException
    {
        ...
        if (!hasOption)
        {
            throw new UnrecognizedOptionException("Unrecognized option: " + arg[0], arg[0]);
        }
