	 ...
	 @Override
	  public Instruction execute (SystemState ss, KernelState ks, ThreadInfo th) {
			 ...
			 if (!th.isFirstStepInsn()) {
				 ...
				 if(prevPartitionCG != null && symbolicVariableExists(objref)) {
			 ...
			 if(numPartitions == 1) {
				 ...
				 if(!concTypeMatch) {
					 ...
					 EquivalenceClass currPart = partitionForTypes.get(currentChoice);
					 currEqObjs.replaceClass(objref,currPart);
					 this.generatingNewConcretization(objref, currEqObjs.getEquivClass(objref).
		 ...
		 //TODO: possibly move to a central helper class with static methods
		 // so all the different bytecode implementations can access it 
		 private boolean symbolicVariableExists(int objref) {
		 ...
		 private int getNumberOfParititions(int objref, KernelState ks) {
			 initializePartitionDataStructs(Integer.toString(objref));
		 ...
		 private void initializePartitionDataStructs(String objIdentifier) {
			 partitionForTypes = new HashMap<Integer, EquivalenceClass>(2);
			 for(int index = 0; index < 2; index++) {
				EquivalenceClass ec = new EquivalenceClass(objIdentifier);
				partitionForTypes.put(index, ec);
			 }
		 }
		 
		 private void generatingNewConcretization(int objref, EquivalenceElem elem,
